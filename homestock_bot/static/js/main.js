...
async function fetchGrafico() {
    try {
        const response = await fetch('/api/dashboard/grafico');
        const data = await response.json();
        ...
        inventoryChartInstance = new Chart(ctx, {
            type: 'doughnut',
            data: {
                labels: data.map(d => d.sala),
                datasets: [{
                    data: data.map(d => d.total),
                    backgroundColor: ['#6366f1', '#8b5cf6', '#3b82f6', '#14b8a6', '#f59e0b', '#ec4899'],
                    borderWidth: 0
                }]
            },
            options: {
...
async function fetchTendencias() {
    try {
        const response = await fetch('/api/stats/tendencias');
        const data = await response.json();
        const ctx = document.getElementById('tendenciasChart');
        if (!ctx) return;
        
        if (tendenciasChartInstance) tendenciasChartInstance.destroy();
        ...
        tendenciasChartInstance = new Chart(ctx, {
            ...
            data: {
                ...
                datasets: [{
                    label: 'Movimientos (últimos 30 días)',
                    data: data.map(d => d.movimientos),
